...
5. If repository retrieval returns `partial` or `not_found` after both primary and configured fallback sources were searched, the root conversation may use web search as an explicit fallback. Do not let any Gemini subagent browse the web.
...
The main conversation owns retrieval and ordinary reasoning. Gemini's optional complex specialist inherits the selected model and is instructed to reason carefully. If it is unavailable, analyze in the main conversation without weakening the guardrails.
